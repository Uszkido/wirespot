# WireSpot Pitch

## One-line Summary

WireSpot is a modern Android hotspot management app for MikroTik RouterOS,
designed for operators who need secure router access, voucher sales, user
control, reporting, printing, and remote management through WireGuard.

## The Problem

Many hotspot operators still manage MikroTik routers with manual WinBox work,
unsafe public router access, scattered voucher records, and separate tools for
printing, reporting, and remote support. This creates slow operations, weak
security, poor sales tracking, and difficult troubleshooting.

## The Solution

WireSpot brings the daily hotspot workflow into one mobile app:

- Connect to MikroTik locally on-site or remotely through WireGuard.
- Manage routers, hotspot users, profiles, sessions, queues, cookies, and IP
  bindings.
- Generate single or batch vouchers with QR codes.
- Print tickets and receipts on 58mm or 80mm Bluetooth thermal printers.
- Track sales, voucher history, and reports.
- Protect the app with PIN, biometric login, encrypted storage, and licensing.

## Why WireSpot Stands Out

- Built for MikroTik hotspot operators, not generic router monitoring.
- RouterOS API client is original and implemented from scratch.
- Remote access is designed around WireGuard instead of exposing RouterOS to
  the public internet.
- Works for both local LAN servicing and remote managed routers.
- Vexel Innovations branding, support, and documentation are built in.
- Professional co-branding lets operators place their own business identity on
  receipts and reports while WireSpot remains powered by Vexel Innovations.
- Premium licensing is planned for Play Store subscriptions and device-bound
  offline licenses.

## Target Users

- Internet cafe operators
- Campus hotspot operators
- Estate and hotel Wi-Fi operators
- Small ISPs
- MikroTik technicians
- Voucher resellers
- Managed Wi-Fi support teams

## Core Value

WireSpot saves operators time, improves security, keeps sales records in one
place, and makes voucher printing/reporting easier from an Android phone.

## Business Model

Recommended model:

- 7-day free full-access trial.
- Monthly and yearly Play Store subscriptions.
- Lifetime device-bound offline licenses for direct APK or enterprise users.
- Professional co-branding for Pro/Enterprise users.
- Optional enterprise/server-validated license plans later.

Recommended product IDs:

- `wirespot_small_monthly`
- `wirespot_pro_monthly`
- `wirespot_pro_yearly`
- `wirespot_lifetime_device`

## Current Development Status

WireSpot already includes the main Android application, RouterOS integration,
hotspot workflows, voucher generation, reporting, settings, local licensing,
WireGuard backend wiring, Bluetooth printing, documentation, and Play Store
preparation files.

Remaining production hardening includes:

- Final real-device WireGuard validation across multiple Android models.
- More printer brand testing for ESC/POS compatibility.
- Production Google Play Billing implementation.
- Server-side license validation.
- Release signing and Play Store internal testing.
- More UI flow and RouterOS command tests.

## Brand

- Product: WireSpot
- Company: Vexel Innovations
- Email: Vexelvision@gmail.com
- Phone: +234(0)7038953065
- Website: https://vexel-innovations.vercel.app/
